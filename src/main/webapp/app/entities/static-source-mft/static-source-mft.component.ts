...
@Component({
    selector: 'jhi-static-source-mft',
    templateUrl: './static-source-mft.component.html'
})
export class StaticSourceMftComponent implements OnInit, OnDestroy {
...
currentAccount: any;
        ...
        if (this.currentSearch) {
            this.staticSourceService.search({
                ...
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<StaticSourceMft[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/static-source-mft'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/static-source-mft', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: StaticSourceMft) {
        return item.id;
    }
